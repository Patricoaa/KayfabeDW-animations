    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-2">
        ...
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto">
          ...
          {seriesPaths.map((s) => (
            <path key={s.name} d={s.path} fill="none" stroke={s.color} strokeWidth={2.5} strokeLinejoin="round" />
          ))}
          {showMarkers &&
            seriesPaths.map((s) =>
              s.points.map((p, i) => (
...
function SingleLine({data, config}: Props) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto">
      ...
      {showMarkers &&
        pts.map((p, i) => <circle key={i} cx={p.x} cy={p.y} r={4} fill={color} stroke="#111" strokeWidth={1.5} />)}
