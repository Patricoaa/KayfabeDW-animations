...
export function LineChart({data, config}: Props) {
  ...
  if (horizontal) {
    ...
    const linePath = smooth
      ? buildSmoothPath(prepared.items.map((p, i) => ({x: toX(p.value), y: toY(i)})))
      : prepared.items.map((p, i) => `${i === 0 ? 'M' : 'L'} ${toX(p.value)} ${toY(i)}`).join(' ');
    ...
    return (
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto">
        ...
        {prepared.items.map((p, i) => (
          <circle key={i} cx={toX(p.value)} cy={toY(i)} r={4} fill={color} stroke="#111" strokeWidth={1.5} />
        ))}
